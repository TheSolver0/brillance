...
window.fbAsyncInit = function() {
    ...
    FB.init({
        appId      : '680551655920659', // FB App ID
...
function saveUserData(userData){
    let base = $('#status').attr('base')+"facebook_login/saveUserData'";
    ...
    $.post(base, {oauth_provider:'facebook', userData: JSON.stringify(userData)}, function(data){ return true; });
}

// Logout from facebook
function fbLogout() {
    let url = $('#status').attr('url');
    FB.logout(function() {
        document.getElementById('fbLink').setAttribute("onclick","fbLogin()");
        //document.getElementById('fbLink').innerHTML = '<img src="<?php echo base_url('assets/template_home/images/fb-login-btn.png'); ?>"/>';
        document.getElementById('fbLink').innerHTML = url;
